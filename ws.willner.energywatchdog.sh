...
SLEEP=10
APPS=( 'Activity Monitor' 'suggestd' 'AppleSpell' 'Messages' 'Calendar' 'CalendarAgent' 'soagent' 'callservicesd' 'airportd' 'cloudphotosd' )
MAX=80.0
...
while true; do
  ...
  for APP in "${APPS[@]}"; do
    ...
    LOAD_LAST="$(cat ${TMPDIR}/EnergyWatchDog_${PID} 2>/dev/null)"
    STATUS="OK"
